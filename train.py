...
def train(args):
    ...
    if args.save:
        if not args.save_dir:
            ...
            models_dir = os.path.join(os.environ['LOG_DIR'], 'nc', date)
        ...
        with torch.no_grad():
            if (epoch + 1) % args.eval_freq == 0:
                ...
                if model.has_improved(best_val_metrics, val_metrics):
                    best_test_metrics, classification_report = model.compute_metrics(
    ...
    if not best_test_metrics:
        model.eval()
        best_emb = model.encode(data['text_features'], data['image_features'], data['adj_train_norm'])
        best_test_metrics, classification_report = model.compute_metrics(best_emb, data, 'test')
    logging.info(" ".join(
        ["Val set results:",
